<template>
    <div class="full-calendar-body">
        ...
        <div class="dates" ref="dates">
            <div class="dates-bg">
                <div class="week-row" v-for="week in currentDates" v-bind:style="{'min-height': calculateHeight(week)}">
            ...
            <div class="dates-events">
                <div class="events-week" v-for="week in currentDates"
                     v-bind:style="{'min-height': calculateHeight(week)}">
                    <div class="events-day" v-for="day in week" track-by="$index"
                         :class="{'today' : day.isToday, 'past': day.isPast, 'not-cur-month' : !day.isCurMonth}"
                         @click.stop="dayClick(day.date, $event)" style="position: relative;">
                        <i v-if="plusSign && !day.isPast" class="fa fa-plus" aria-hidden="true"></i>
                        <div class="event-box">
                        ...
                        <p class="day-number"><span>{{day.monthDay}}</span></p>
                    </div>
                </div>
            </div>
...
<script type="text/babel">
    ...
    export default {
        ...
        computed: {
            currentDates() {
                let currentDates = this.getCalendar();
                currentDates = [];
                // TODO: сделать то же самое, и вынести в отдельный класс сортировки
                for (let i = 0; i < currentDates.length; i++) {
                    let week = currentDates[i];
                    for (let j = 0; j < week.length; j++) {
                        ...
                        if (day.events.length > 0) {
                            day.events.sort(function (a, b) {
                                ...
                                return (moment(a.entity.time_start, 'HH:mm').format('x') > moment(b.entity.time_start, 'HH:mm').format('x')) ? 1 : 0;
                            });
                        }
        ...
        methods: {
            calculateHeight(week) {
                let max = 0;
                for (let i = 0; i < week.length; i++) {
                    if (max < week[i].events.length) {
                        max = week[i].events.length;
                    }
                }
                let height = 90;
                let limit = 3;
                let item = 30;
                return (max > limit) ? (max * item) + 'px' : height + 'px';
            },
                ...
                for (let perWeek = 0; perWeek < 6; perWeek++) {
                    ...
                    for (let perDay = 0; perDay < 7; perDay++) {
                        week.push({
                            ...
                            isPast: (new Date(now.getFullYear(), now.getMonth(), now.getDate())) > startDate,
